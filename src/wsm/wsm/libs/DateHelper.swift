...
public extension Calendar {
    public static var calendarUS: Calendar {
        var gregorian = Calendar(identifier: .gregorian)
        gregorian.timeZone = TimeZone(identifier: "America/New_York")!
        gregorian.locale = Locale(identifier: "America/New_York")
        gregorian.firstWeekday = 1
        return gregorian
    }

    public var todayOfCalendar: Date {
        var now = Date()
        var nowComponents = DateComponents()

        nowComponents.year = self.component(.year, from: now)
        nowComponents.month = self.component(.month, from: now)
        nowComponents.day = self.component(.day, from: now)
        nowComponents.hour = self.component(.hour, from: now)
        nowComponents.minute = self.component(.minute, from: now)
        nowComponents.second = self.component(.second, from: now)
        nowComponents.timeZone = self.timeZone
        now = self.date(from: nowComponents)!
...
public extension Date {
    ...
    func compare(_ comparison:DateComparisonType) -> Bool {
        ...
        case .isWeekend:
            let range = currentCalendar.maximumRange(of: Calendar.Component.weekday)!
    ...
    func adjust(_ component:DateComponentType, offset:Int) -> Date {
        ...
        return currentCalendar.date(byAdding: dateComp, to: self)!
    }
    ...
    func numberOfDaysInMonth() -> Int {
        let range = currentCalendar.range(of: Calendar.Component.day, in: Calendar.Component.month, for: self)!
    ...
    internal static func components(_ fromDate: Date) -> DateComponents {
        return Calendar.calendarUS.dateComponents(Date.componentFlags(), from: fromDate)
    }
...
public enum TimeZoneType {
    case local, utc, useOfCurrentCalendar, custom(identifier: String)
    var timeZone:TimeZone {
        switch self {
        case .local: return NSTimeZone.local
        case .utc: return TimeZone(secondsFromGMT: 0)!
        case .useOfCurrentCalendar: return Calendar.calendarUS.timeZone
        case .custom(let identifier): return TimeZone(identifier: identifier)!
        }
    }
}

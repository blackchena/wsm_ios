...
{
    if ([self.calendar.dataSource respondsToSelector:@selector(numberOfHeadPlaceholdersForMonth:)]) {
        return [self.calendar.dataSource numberOfHeadPlaceholdersForMonth:month];
    }
    else {
...
{
    if ([self.calendar.dataSource respondsToSelector:@selector(numberOfRowsInMonth:)]) {
        return [self.calendar.dataSource numberOfRowsInMonth:month];
    } else {
        if (!month) return 0;
...
- (NSInteger)numberOfRowsInSection:(NSInteger)section
{
    ...
    NSInteger numberOfRowInMonthResult = [self numberOfRowsInMonth:month];
    
    return numberOfRowInMonthResult;
}
